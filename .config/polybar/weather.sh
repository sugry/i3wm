...
wget -q -O- "$URL" | awk -F\' '/acm_RecentLocationsCarousel\.push/{print $10"°", $13}'| head -1 | sed 's/,  text:"//g' | rev | cut -c 5- | rev | sed 's/.*/\L&/' | sed 's/ясно//g' | sed 's/солнечно//g' | sed 's/слабый туман//g' | sed 's/слабый снег и туман/ /g' | sed 's/слабый снегопад//g' | sed 's/небольшой снег//g' | sed 's/сильный снегопад//g' | sed 's/снегопад//g' | sed 's/снег//g' | sed 's/значительная облачность//g' | sed 's/облачно с прояснениями//g' | sed 's/малооблачно//g' | sed 's/облачно//g'
#-12° слабый снегопад                  
# пишем текст погоды в файл для вывода в notify-send по клику на модуле

#if test -f "/home/s-adm/.config/polybar/weatmp"; then rm /home/s-adm/.config/polybar/weatmp;fi
#touch /home/s-adm/.config/polybar/weatmp

# То же, но быстрее и короче. "Обнуляет" файл. Может использоваться вместо touch для создания новых файлов.
# http://kubuntu.ru/node/4381

cat /dev/null > /home/s-adm/.config/polybar/weatmp
wget -q -O- "$URL" | awk -F\' '/acm_RecentLocationsCarousel\.push/{print $10"°", $13}'| head -1 | sed 's/,  text:"//g' | rev | cut -c 5- | rev | sed 's/.*/\L&/' | sed 's/-/минус /g' >> /home/s-adm/.config/polybar/weatmp
